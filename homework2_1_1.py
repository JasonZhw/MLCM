...
def get_model(activation_fn):
    return SineNet(input_dim=1, hidden_dim=50, output_dim=1, activation_fn=activation_fn).to(device)
...
for lr in learning_rates:
    ...
    # Plotting loss curves
    plot_loss_curves(train_losses, val_losses, title=f"Training and Validation Loss over Epochs with learning rate {lr}")
...
for lambda_l2 in lambdas:
    ...
    model, train_losses, val_losses = train(train_loader, val_loader, learn_rate, epochs, nn.Tanh(), lambda_l2=lambda_l2)

    # Plotting loss curves
    plot_loss_curves(train_losses, val_losses, title=f"Training and Validation Loss over Epochs with L2 regularization (lambda={lambda_l2})")
